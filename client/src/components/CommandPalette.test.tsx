...
describe('CommandPalette', () => {
  it('keeps focus in the combobox while arrows announce and Enter opens the selected page', () => {
    ...
    expect(onNavigate).toHaveBeenCalledExactlyOnceWith('roadmap');
  ...
  it('finds every page immediately by label, route or description without waiting for records', () => {
    vi.useFakeTimers();
    ...
    expect(screen.getByRole('option', {name: /Access reviews/})).toBeVisible();
  ...
  it('offers recommended page jumps in a labeled dialog before typing', () => {
    ...
    fireEvent.click(within(pages).getByRole('option', {name: /SOC 2 roadmap/}));
    expect(onNavigate).toHaveBeenCalledExactlyOnceWith('roadmap');
    expect(onClose).toHaveBeenCalledOnce();
    expect(api.get).not.toHaveBeenCalled();
  });
