...
from .coverage_ops import coverage_router
from .auth_ops import auth_router
...
from .auditor_ops import auditor_router as auditor_portal_router
...
def create_app(data_dir: Path | str | None = None, auto_seed: bool = True) -> FastAPI:
    ...
    app = FastAPI(title='tofromGRC', version='0.20.0', docs_url=None, redoc_url=None)
    ...
    @app.get('/api/health')
    def health():
        return {'status': 'ok', 'version': '0.20.0', 'storage': 'sqlite'}
    ...
    app.include_router(auditor_portal_router(store))
    ...
    app.include_router(coverage_router(store))
    app.include_router(auth_router(store))
